...
const { isFname, isLname, isEmail, isPhone, isPassword, isStreet, isCity, isPincode, removeSpaces, trimAndUpperCase } = require('../validators/validator');
...
const validateUserPut = async function (req, res, next) {
    try {
        ...
        arrKeys.forEach((x, i) => {
            if (req.body[x]) {
                if (i < 2) objUpdate[x] = trimAndUpperCase(req.body[x]);
                if (i >= 2 && i < 7) objUpdate[x] = removeSpaces(req.body[x]);
