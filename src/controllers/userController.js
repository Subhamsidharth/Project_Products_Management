...
const createUser = async function (req, res) {
    try {
        ...
        if (!Object.keys(req.body).length) return res.status(400).send({ status: false, message: "Bad Request, Please enter the details in the request body.❌🛑" });
        ...
        } catch (err) { return res.status(400).send({ status: false, message: "please send addresss in proper format so that JSON parsing can be performed, & address must includes mandatory street, city & pincode for shipping and billing" }) }

        if (!address.shipping) error.shippingError = "enter the mandatory shipping address with street, city & pincode";
        if (!address.billing) error.billingError = "enter the mandatory billing address with street, city & pincode";
